...
"""
1변수 방정식의 해
어떤 (비선형) 함수 f(x) 값이 0이 되도록 만드는 x를 찾음
"""
# 컴퓨터의 메모리에는 원래는 제한된 자릿수의 2진수만 저장하 수 있음
# 실수를 적용하려면 오차가 발생하게 됨
# epsilon 은 허용되는 오차 범위를 의미함
#
#
epsilon_global = 1e-4
...
def sequential(f, x0, delta_x=1e-6, epsilon=epsilon_global, b_verbose=False):
    ...
    xi = float(x0)
    # delta_x 의 의미는
    # "아직 답을 찾지 못했을 떄 xi를 얼마만큼 증가시킬 것인가"
    ...
    while True:
        ...
        # x 를 delta_x 만큼 증가시킴
        xi += delta_x
        # 반복문이 한번 실행 되었으므로 counter를 1 증가 시킴
        counter += 1
...
def bisection(f, xl, xh, epsilon=epsilon_global, b_verbose=False):
    ...
    f(xl) 과 f(xh) 의 부로가 반대이면 xh를 xn으로 옮김
        이렇게 하면 계속 f(xl) 과 f(xh)의 부호가 반대임
    그렇지 않으면 xl을 xn으로 옮김
         f(xl) 과 f(xh)의 부호가 반대일 것임
...
def newton(f, df, x0, epsilon=epsilon_global, b_verbose=False):
    ...
    비선형 함수인 f(x)의 xi 지점에서의 접선의 방적식의 근을 구함
